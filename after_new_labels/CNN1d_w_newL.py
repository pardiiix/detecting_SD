...
#reading the new labels comment file
with open('new_labels_pos.txt', 'r') as file:
    sd = file.readlines()

with open('new_labels_neg.txt', 'r') as file:
...
history = model.fit(X_train, y_train, batch_size=32, epochs=5, verbose = 1, validation_split =0.2) #verbose =1 : see trainig progress for each epoch


score = model.evaluate(X_dev, y_dev, verbose = 1)

model.save("saved_cnn1D_model_after_newL.h5")
