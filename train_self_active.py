...
def create_semi_cnn_model(file_name):
    ...
    semi_df = semi_df.sample(frac=1, random_state = 10) #shuffling the rows
    semi_df.reset_index(inplace = True, drop = True)
    ...
    max_sent_len = 200
    max_vocab_size = 4000
    word_seq = [text_to_word_sequence(comment) for comment in semi_df['comments']]
    # print(word_seq)
